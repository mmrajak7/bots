...
class OrderManager:
            ...
            for order in active_pending:
                ...
                if gtt is None:
                    ...
                    if fill_result:
                        ...
                        # Only alert once per order (check last_error to dedup)
    ...
    def _check_if_gtt_triggered(self, order: OpenOrder) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            try:
                ...
                for h in holdings:
                    if h.get('tradingsymbol') == order.script:
                        total_qty = h.get('quantity', 0) + h.get('t1_quantity', 0)
                        if total_qty > 0:
                            avg_price = h.get('average_price', order.limit_price)
                            logger.info(
                                f"GTT fill detected via holdings: {order.script} "
                                f"qty={total_qty} @ {avg_price}"
                            )
                            return self._process_holdings_fill(order, avg_price, total_qty)
            except Exception as e:
                logger.warning(f"Could not check holdings for {order.script}: {e}")
    ...
    def _process_holdings_fill(
        self, order: OpenOrder, fill_price: float, fill_qty: int
    ) -> Optional[Dict[str, Any]]:
        """
        Process a fill detected via holdings (no order_id available).
        Creates position directly from holdings data.
        """
        session = get_session()
        try:
            ...
            position = self._create_position_from_order(
                order, fill_price, fill_qty, session
            )
            ...
            telegram.send_alert(
                f"<b>Position Opened (Reconciled)</b>\n\n"
                f"{order.script} @ {fill_price:,.2f}\n"
                f"Qty: {fill_qty}\n"
                f"Initial SL: {position.initial_sl:,.2f}\n"
                f"<i>Detected via holdings check</i>"
            )
            ...
            logger.error(f"Error processing holdings fill for {order.script}: {e}")
            session.rollback()
            return None
        finally:
            session.close()
