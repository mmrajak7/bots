...
# Load credentials from cred.env (NEVER hardcode secrets)
def _load_credentials():
    """Load credentials from CROCODILE/config/cred.env file."""
    cred_file = Path(__file__).parent / "config" / "cred.env"
    if not cred_file.exists():
        raise FileNotFoundError(
            f"Credentials file not found: {cred_file}\n"
            f"Copy cred.env.template to cred.env and fill in your credentials."
        )
    creds = {}
    ...
    required = ['ZERODHA_API_KEY', 'ZERODHA_API_SECRET', 'ZERODHA_USER_ID',
                'ZERODHA_PASSWORD', 'ZERODHA_TOTP_KEY']
    missing = [k for k in required if not creds.get(k)]
    if missing:
        raise ValueError(f"Missing credentials in {cred_file}: {', '.join(missing)}")
    ...
    return {
        ...
        "totp_secret": creds['ZERODHA_TOTP_KEY'],
    }
