...
def SuperTrend(df, period, multiplier, ohlc=['Open', 'High', 'Low', 'Close']):
    ...
    close = df[ohlc[3]].values

    final_ub = np.zeros(n)
    final_lb = np.zeros(n)
    supertrend = np.zeros(n)

    # Compute final upper and lower bands using numpy arrays (much faster than .iat)
...
def RSI(df, base="Close", period=21):
    ...
    df['RSI_' + str(period)] = df['RSI_' + str(period)].fillna(0)
